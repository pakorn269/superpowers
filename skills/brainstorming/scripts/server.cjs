...
let cachedHtmlResponse = buildHtmlResponse(null);

function buildHtmlResponse(raw) {
  let html = raw ? (isFullDocument(raw) ? raw : wrapInFrame(raw)) : WAITING_PAGE;
...
async function updateNewestScreen() {
  try {
    ...
    if (cachedNewestScreen) {
      ...
      cachedHtmlResponse = buildHtmlResponse(raw);
    } else {
      cachedHtmlResponse = buildHtmlResponse(null);
    }
  } catch (err) {
